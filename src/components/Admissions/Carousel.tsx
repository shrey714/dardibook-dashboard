...
>(
  (
    ...
    const [activeIndex, setActiveIndex] = useState<number>(15);
    ...
    useEffect(() => {
      if (!emblaMainApi) return;
      const handleInit = () => {
        emblaMainApi.scrollTo(15); // Scroll to "today"
        onSelect();
      };
      if (emblaMainApi) {
        handleInit();
      }
      emblaMainApi.on("init", handleInit);
      emblaMainApi.on("select", onSelect);
      emblaMainApi.on("reInit", handleInit);

      return () => {
        emblaMainApi?.off("init", handleInit);
        emblaMainApi?.off("select", onSelect);
        emblaMainApi?.off("reInit", handleInit);
      };
...
>(({ className, index, highlighted, setActiveTab, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      <div
        className={`relative aspect-square border h-10 sm:h-14 text-center flex items-center justify-center rounded-xl text-xs sm:text-sm transition-[color,opacity] ${
          isSlideActive ? "border-foreground ring-[3px] ring-foreground/50 bg-input/30 text-foreground" : "text-muted-foreground"
        } ${highlighted ? "bg-primary/30 border-primary ring-primary/50 text-primary" : ""}`}
      >
        {children}
      </div>
    </div>
